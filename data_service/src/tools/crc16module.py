...
class CRC16NotMatchException(Exception):
    pass
...
def check_crc(full_package: bytes) -> bool:
    """Validate signed bytes package."""
    if not crc16(full_package[:-2]) == full_package[-2:]:
        raise CRC16NotMatchException(f'Не совпадает CRC16:\n    - Ожидается {add_crc(full_package[:-2])}\n    - Получено: {full_package[-2:]}')
